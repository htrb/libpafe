...
int pasori_packet_write(pasori *p, const uint8 *data, int *size);
int pasori_packet_read(pasori * p, uint8 * data, int *size);
int pasori_write(pasori *p, const uint8 *data, int *size);
int pasori_read(pasori *p, uint8 *data, int *size);
int pasori_reset(pasori * p);
int pasori_version(pasori *p, int *v1, int *v2);
int pasori_type(const pasori *p);

int pasori_test(pasori *p, int code, const uint8 *data, const int *size, uint8 *rdata, int *rsize);
int pasori_test_echo(pasori *p, const uint8 *data, const int *size);
int pasori_test_eprom(pasori *p);
int pasori_test_ram(pasori *p);
int pasori_test_cpu(pasori *p);
int pasori_test_polling(pasori *p);
int pasori_list_passive_target(pasori *pp, const unsigned char *payload, int *size);
